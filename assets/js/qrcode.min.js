/**
 * EasyQRCodeJS / QRCode.js for WP Agency Toolkit
 * Lightweight Standalone QR Code Generator with SVG / Canvas rendering & Logo support
 */
(function(root, factory) {
    ...
    QRCode.prototype.getSVG = function() {
        if (!this._oQRCode) return '';
        var nCount = this._oQRCode.getModuleCount();
        var width = this._htOption.width;
        var height = this._htOption.height;
        var nWidth = (width / nCount).toFixed(2);
        var nHeight = (height / nCount).toFixed(2);

        var svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + width + ' ' + height + '" width="' + width + '" height="' + height + '">';
        svg += '<rect width="100%" height="100%" fill="' + this._htOption.colorLight + '"/>';
        svg += '<path fill="' + this._htOption.colorDark + '" d="';
        for (var row = 0; row < nCount; row++) {
            for (var col = 0; col < nCount; col++) {
                if (this._oQRCode.isDark(row, col)) {
                    var x = (col * nWidth).toFixed(2);
                    var y = (row * nHeight).toFixed(2);
                    svg += 'M' + x + ',' + y + 'h' + nWidth + 'v' + nHeight + 'h-' + nWidth + 'z ';
                }
            }
        }
        svg += '"/>';
