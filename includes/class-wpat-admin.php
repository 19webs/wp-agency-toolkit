...
&lt;/script&gt;</code></pre>
	...
	public function render_client_studio_content( $settings ) {
		...
		$current_tab    = isset( $_GET['studio_tab'] ) ? sanitize_key( $_GET['studio_tab'] ) : 'settings';
		...
		$compatible_pts = WPAT_Client_Studio::get_supported_post_types();
		...
		$hover_effect   = isset( $settings['studio_hover'] ) ? $settings['studio_hover'] : 'subtle';
		$pills_enabled  = ! isset( $settings['studio_pills'] ) || '1' === (string) $settings['studio_pills'];
		$thumbs_enabled = ! isset( $settings['studio_show_thumbs'] ) || '1' === (string) $settings['studio_show_thumbs'];
		$zoom_enabled   = ! isset( $settings['studio_thumb_zoom'] ) || '1' === (string) $settings['studio_thumb_zoom'];
		$sticky_enabled = isset( $settings['studio_sticky_header'] ) && '1' === (string) $settings['studio_sticky_header'];
		...
		<div class="wpat-module-card wpat-studio-wrap">
			<div class="wpat-module-header" style="margin-bottom: 20px;">
				<div class="wpat-module-info">
					<h3>Diseño SaaS para Entradas, Productos & CPTs <span class="wpat-badge" style="background:#2563eb; color:#fff;">Experiencia SaaS</span></h3>
					<p>Transforma directamente el menú nativo de WordPress (listados edit.php y editor post.php) en un espacio visual, limpio y amigable estilo Notion/Ghost.</p>
				</div>
			...
			<div class="wpat-module-body" style="display: block; padding: 22px;">
				...
				<div style="display: flex; gap: 8px; border-bottom: 2px solid #e2e8f0; margin-bottom: 24px; flex-wrap: wrap;">
					<a href="<?php echo esc_url( add_query_arg( 'studio_tab', 'settings', $base_url ) ); ?>" style="padding: 10px 16px; font-weight: 700; font-size: 13.5px; text-decoration: none; border-bottom: 3px solid <?php echo ( 'settings' === $current_tab ) ? '#2563eb' : 'transparent'; ?>; color: <?php echo ( 'settings' === $current_tab ) ? '#2563eb' : '#64748b'; ?>;">
						⚙️ Configuración & Activación en WordPress
					</a>
					<a href="<?php echo esc_url( add_query_arg( array( 'studio_tab' => 'explorer', 'studio_action' => 'list', 'studio_pt' => $current_pt ), $base_url ) ); ?>" style="padding: 10px 16px; font-weight: 700; font-size: 13.5px; text-decoration: none; border-bottom: 3px solid <?php echo ( 'explorer' === $current_tab ) ? '#2563eb' : 'transparent'; ?>; color: <?php echo ( 'explorer' === $current_tab ) ? '#2563eb' : '#64748b'; ?>;">
						✍️ Explorador & Editor Studio
					</a>
				</div>
				...
				<?php if ( 'settings' === $current_tab ) : ?>
					...
					<form method="post" action="">
						<?php wp_nonce_field( 'wpat_save_settings_action', 'wpat_settings_nonce' ); ?>
						<input type="hidden" name="wpat_save_settings_btn" value="1" />
						<input type="hidden" name="wpat_saving_module" value="client-studio" />

						<!-- Banner explicativo de cómo funciona en WP -->
						<div style="background: linear-gradient(135deg, #eff6ff 0%, #f8fafc 100%); border: 1px solid #bfdbfe; border-radius: 12px; padding: 18px 22px; margin-bottom: 24px;">
							<div style="display: flex; justify-content: space-between; align-items: center; gap: 16px; flex-wrap: wrap;">
								<div>
									<h4 style="margin: 0 0 4px 0; color: #1e3a8a; font-size: 14.5px; font-weight: 800;">
										🎯 Integración Directa con el Menú Nativo de WordPress
									</h4>
									<p style="margin: 0; font-size: 12.5px; color: #3b82f6;">
										Al activar este módulo, cuando tú o tus clientes hagan clic en <strong>Entradas</strong>, <strong>Páginas</strong>, <strong>Productos</strong> o <strong>CPTs</strong> en el menú lateral izquierdo de WordPress, se cargará automáticamente la interfaz SaaS moderna sin necesidad de venir a esta pantalla.
									</p>
								</div>
								<div style="display: flex; gap: 8px;">
									<a href="<?php echo esc_url( admin_url( 'edit.php' ) ); ?>" class="button button-secondary" style="height: 36px; line-height: 34px; font-weight: 700; border-radius: 6px;">
										Probar Listado (edit.php) →
									</a>
									<a href="<?php echo esc_url( admin_url( 'post-new.php' ) ); ?>" class="button button-primary" style="background: #2563eb; border-color: #1d4ed8; font-weight: 700; height: 36px; line-height: 34px; padding: 0 16px; border-radius: 6px;">
										Probar Editor (post-new.php) →
									</a>
								</div>
							</div>
						</div>

						<!-- SECCIÓN: POST TYPES HABILITADOS -->
						<div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; padding: 20px; margin-bottom: 24px;">
							<h4 style="margin: 0 0 6px 0; font-size: 14px; font-weight: 800; color: #0f172a;">
								🎯 Tipos de Contenido (Post Types) con Experiencia SaaS
							</h4>
							<p style="margin: 0 0 16px 0; font-size: 12.5px; color: #64748b;">
								Selecciona en qué pantallas de WordPress se aplicará la interfaz moderna:
							</p>

							<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px;">
								<?php
								foreach ( $compatible_pts as $pt_name => $pt_item ) :
									$is_checked = in_array( $pt_name, $enabled_pts, true );
									$icon = '📝';
									if ( 'page' === $pt_name ) {
										$icon = '📄';
									...
									?>
									<label style="display: flex; align-items: center; gap: 8px; background: #ffffff; border: 1px solid <?php echo $is_checked ? '#2563eb' : '#cbd5e1'; ?>; padding: 10px 14px; border-radius: 8px; cursor: pointer; transition: all 0.15s ease;">
										<input type="checkbox" name="wpat_settings[studio_post_types][]" value="<?php echo esc_attr( $pt_name ); ?>" <?php checked( $is_checked ); ?> />
										<span style="font-weight: 700; font-size: 13px; color: #1e293b;">
											<?php echo esc_html( $icon . ' ' . $pt_item->labels->name ); ?>
										</span>
										<span style="font-size: 11px; color: #94a3b8; margin-left: auto;">
											(<?php echo esc_html( $pt_name ); ?>)
										</span>
						...
						<div style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 10px; padding: 20px; margin-bottom: 24px;">
							<h4 style="margin: 0 0 6px 0; font-size: 14px; font-weight: 800; color: #0f172a;">
								👥 Asignación por Roles de Usuario
							</h4>
							<p style="margin: 0 0 16px 0; font-size: 12.5px; color: #64748b;">
								Elige a qué roles de usuario se les activará la experiencia SaaS simplificada:
							</p>

							<div style="display: flex; gap: 14px; flex-wrap: wrap;">
								<label style="display: flex; align-items: center; gap: 6px; font-size: 13px; font-weight: 600; cursor: pointer;">
									<input type="checkbox" name="wpat_settings[studio_roles][]" value="all" <?php checked( in_array( 'all', $target_roles, true ) ); ?> />
									<span>🌐 Todos los usuarios (Recomendado)</span>
								</label>
								<label style="display: flex; align-items: center; gap: 6px; font-size: 13px; font-weight: 600; cursor: pointer;">
									<input type="checkbox" name="wpat_settings[studio_roles][]" value="editor" <?php checked( in_array( 'editor', $target_roles, true ) ); ?> />
									<span>✍️ Editores</span>
								</label>
								<label style="display: flex; align-items: center; gap: 6px; font-size: 13px; font-weight: 600; cursor: pointer;">
									<input type="checkbox" name="wpat_settings[studio_roles][]" value="author" <?php checked( in_array( 'author', $target_roles, true ) ); ?> />
									<span>📝 Autores</span>
								</label>
								<label style="display: flex; align-items: center; gap: 6px; font-size: 13px; font-weight: 600; cursor: pointer;">
									<input type="checkbox" name="wpat_settings[studio_roles][]" value="administrator" <?php checked( in_array( 'administrator', $target_roles, true ) ); ?> />
						...
						<div style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 10px; padding: 20px; margin-bottom: 24px;">
							<h4 style="margin: 0 0 16px 0; font-size: 14px; font-weight: 800; color: #0f172a;">
								🎨 Opciones Visuales de las Tablas (edit.php)
							</h4>

							<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px; margin-bottom: 18px;">
								<div>
									<label style="display: block; font-weight: 700; font-size: 13px; margin-bottom: 6px;">📏 Espaciado de Filas</label>
									<select name="wpat_settings[studio_density]" style="width: 100%; height: 38px; border-radius: 6px;">
										<option value="comfortable" <?php selected( $density, 'comfortable' ); ?>>Holgado / Cómodo (SaaS 14px padding)</option>
										<option value="compact" <?php selected( $density, 'compact' ); ?>>Compacto (Mayor densidad)</option>
									</select>
								</div>
								<div>
									<label style="display: block; font-weight: 700; font-size: 13px; margin-bottom: 6px;">✨ Efecto al Pasar el Ratón</label>
									<select name="wpat_settings[studio_hover]" style="width: 100%; height: 38px; border-radius: 6px;">
										<option value="subtle" <?php selected( $hover_effect, 'subtle' ); ?>>Sutil (Cambio suave de fondo)</option>
										<option value="bordered" <?php selected( $hover_effect, 'bordered' ); ?>>Borde de Acento Azul</option>
										<option value="shadow" <?php selected( $hover_effect, 'shadow' ); ?>>Sombra Flotante</option>
									</select>
								</div>
							</div>

							<div style="border-top: 1px solid #f1f5f9; padding-top: 16px; display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 14px;">
								<label style="display: flex; align-items: center; gap: 8px; font-size: 12.5px; font-weight: 600; cursor: pointer;">
									<input type="checkbox" name="wpat_settings[studio_pills]" value="1" <?php checked( $pills_enabled ); ?> />
									<span>Badges de Estado de Publicación (Pills en verde/ámbar/azul)</span>
								</label>
								<label style="display: flex; align-items: center; gap: 8px; font-size: 12.5px; font-weight: 600; cursor: pointer;">
									<input type="checkbox" name="wpat_settings[studio_show_thumbs]" value="1" <?php checked( $thumbs_enabled ); ?> />
									<span>Columna de Imagen Destacada con previsualización</span>
								</label>
								<label style="display: flex; align-items: center; gap: 8px; font-size: 12.5px; font-weight: 600; cursor: pointer;">
									<input type="checkbox" name="wpat_settings[studio_thumb_zoom]" value="1" <?php checked( $zoom_enabled ); ?> />
									<span>Zoom Flotante al pasar el cursor sobre la miniatura</span>
								</label>
								<label style="display: flex; align-items: center; gap: 8px; font-size: 12.5px; font-weight: 600; cursor: pointer;">
									<input type="checkbox" name="wpat_settings[studio_sticky_header]" value="1" <?php checked( $sticky_enabled ); ?> />
									<span>Cabecera Fija al hacer scroll (Sticky Header)</span>
								</label>
							</div>
						</div>
						...
						<!-- BOTÓN GUARDAR AJUSTES -->
						<div style="display: flex; justify-content: flex-end;">
							<button type="submit" class="button button-primary" style="background: #2563eb; border-color: #1d4ed8; font-weight: 700; height: 38px; padding: 0 22px; font-size: 13.5px; border-radius: 6px;">
								Guardar Ajustes de Client Studio
							</button>
						</div>
				...
				<?php else : ?>
					<!-- ========================================================= -->
					<!-- PESTAÑA 2: EXPLORADOR Y EDITOR INTEGRADO                   -->
					<!-- ========================================================= -->
					<?php
					...
					<div class="wpat-studio-card" style="padding: 14px 18px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center; gap: 14px; flex-wrap: wrap;">
						<div style="display: flex; gap: 6px; flex-wrap: wrap;">
							<?php foreach ( $compatible_pts as $pt_key => $p_obj ) : ?>
								...
								<a href="<?php echo esc_url( add_query_arg( array( 'studio_tab' => 'explorer', 'studio_action' => 'list', 'studio_pt' => $pt_key ), $base_url ) ); ?>" style="padding: 6px 12px; border-radius: 8px; font-size: 12.5px; font-weight: 700; text-decoration: none; <?php echo $is_active_pt ? 'background: #2563eb; color: #fff;' : 'background: #f1f5f9; color: #475569;'; ?>">
									<?php echo esc_html( $p_obj->labels->name ); ?>
								</a>
							<?php endforeach; ?>
						</div>

						<a href="<?php echo esc_url( admin_url( 'post-new.php' . ( 'post' !== $current_pt ? '?post_type=' . $current_pt : '' ) ) ); ?>" class="button button-primary" style="background: #2563eb; border-color: #1d4ed8; font-weight: 700; height: 34px; line-height: 32px; border-radius: 6px;">
							+ Añadir <?php echo esc_html( $pt_singular ); ?> en WordPress
						</a>
					</div>
					...
					<div class="wpat-studio-card" style="overflow: hidden;">
						<?php if ( $query->have_posts() ) : ?>
							<div style="display: flex; flex-direction: column;">
								<?php while ( $query->have_posts() ) : $query->the_post(); ?>
									...
									?>
									<div class="wpat-studio-item-row" style="padding: 14px 18px; display: flex; justify-content: space-between; align-items: center; gap: 16px; border-bottom: 1px solid #f1f5f9;">
										<div style="display: flex; align-items: center; gap: 14px; min-width: 0;">
											...
											<div style="min-width: 0;">
												...
												<h4 style="margin: 0; font-size: 13.5px; font-weight: 700; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
						...
						<?php else : ?>
							<div style="text-align: center; padding: 40px 20px; color: #64748b;">
								...
								<strong>No se encontraron <?php echo esc_html( strtolower( $pt_label ) ); ?>.</strong>
							</div>
						<?php endif; ?>
					</div>
